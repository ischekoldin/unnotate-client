...
const NotesApp = ({ location }) => {
    ...
    const refreshAccessToken = useCallback (async () => {
        ...
        try {
            ...
            return "token fetch success"

        } catch (err) {
            return new Error("token fetch fail");
        }
    ...
    // in case we came straight to /notes
    if (!token) {
        ...
        (async() => {
            ...
            try {
                 ...
                 if (response !== "token fetch success") {
                     history.push("/");
                 } else {
                     history.push("/notes");
                 }
    ...
    const updateUserName = useCallback ( () => {
        ...
        if (!username) {
            ...
            const userNameFromCookie = document.cookie
                .replace(/(?:(?:^|.*;\s*)unnotateUserName\s*=\s*([^;]*).*$)|^.*$/, "$1");


            if (userNameFromCookie) {
                dispatch({
                    type: "user/set",
                    payload: userNameFromCookie
                });
            }
    ...
    }, [dispatch, token, username, ENDPOINT]);
